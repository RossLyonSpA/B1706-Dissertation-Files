...
df <- df %>% 
  mutate(
    ...
    SS_Unreturned = ifelse(
      is.na(Second_Outcome),
      NA,
      Second_Outcome %in% c("Ace, Service Winner") | (Second_Outcome == "In-Play" & Rally_Length == 1)),  #[NEEDS ATTENTION!]
    ...
    Break_Chance_0015 = Pts == "0-15",
    Break_Chance_1530 = Pts == "15-30",
    Break_Chance_4040 = Pts == "40-40",
...
Create_Summary_Table <- function(df) {
  ...
  summary_df <- df %>% 
    ...
    summarise(
      ...
      # % of 2nd serves that were not returned in court
      Game_SS_Unreturned_N = sum(SS_Unreturned, na.rm = TRUE),
      Game_SS_Unreturned_Pct = ifelse(
        Game_SS_Points == 0,
        NA,
        sum(Game_SS_Unreturned_N, na.rm = TRUE) / Game_SS_Points * 100),
      ...
      # Percentage 1st serve rallies won by server under 5 shots
      ...
      # Percentage 1st serve rallies won by server between 5 and 9 shots
      ...
      # Percentage 1st serve rallies won by server over 9 shots
...
Create_Rolling_Metrics <- function (df) {
  ...
  df <- df %>% 
    ...
    mutate(
      ...
      # Total number of points in a three game window
      ...
      # 3 game % of 1st serve points won
      ...
      # 3 game % of 2nd serve points won
      ...
      # 3 game % of 2nd serves that were not returned in court
      Roll3_SS_Unreturned_N = slide_dbl(Game_SS_Unreturned_N, sum, .before = 2, .complete = FALSE, na.rm = TRUE),
      Roll3_SS_Unreturned_Pct = ifelse(
        Roll3_SS_Points == 0,
        NA,
        (Roll3_SS_Unreturned_N / Roll3_SS_Points) * 100
      ),
...
testing_data_for_later <- bind_rows(list_summaries1)
...
rm(atp_charting_points, code_lookup, list_summaries, list_summaries1, match_dfs,
   match_dfs1, match_dfs2, match_dfs3, match_dfs4, match_dfs5)
...
full_data2 <- full_data %>% 
  select(-c(Game_FS_Won_Pct, Game_SS_Won_Pct))
...
########################################
# FIND AND BEST THRESHOLD VIA ROC CURVE
########################################
...
shap_importance <- shap_values2$mean_shap_score
...
testing_matches_final <- testing_data_for_later %>% 
  filter(Match_ID %in% testing_matches) %>% 
  select(-c(Game_FS_Won_N, Game_FS_Won_Pct, Game_SS_Won_N, Game_SS_Won_Pct))
...
Prediction_Errors <- testing_matches_final %>% 
...
view(Prediction_Errors)




# Successful Predictions of Holds
Prediction_Hold_Correct <- testing_matches_final %>% 
  filter(Game_Outcome == "HOLD" & Pred_Game_Outcome == "HOLD")


# Rename Server column to player names
Prediction_Hold_Correct <- Prediction_Hold_Correct %>%
  mutate(Svr = case_when(
    Match_ID == "OLY2012F_MURvFED" & Svr == "A" ~ "MURRAY",
    Match_ID == "OLY2012F_MURvFED" & Svr == "B" ~ "FEDERER",
    Match_ID == "WIM2012F_FEDvMUR" & Svr == "A" ~ "FEDERER",
...
view(Prediction_Hold_Correct)


# Successful Predictions of Breaks
Prediction_Break_Correct <- testing_matches_final %>% 
  filter(Game_Outcome == "BREAK" & Pred_Game_Outcome == "BREAK")

# Rename Server column to player names
Prediction_Break_Correct <- Prediction_Break_Correct %>%
  mutate(Svr = case_when(
    Match_ID == "OLY2012F_MURvFED" & Svr == "A" ~ "MURRAY",
    Match_ID == "OLY2012F_MURvFED" & Svr == "B" ~ "FEDERER",
    Match_ID == "WIM2012F_FEDvMUR" & Svr == "A" ~ "FEDERER",
...
view(Prediction_Break_Correct)

# Summary Stats for table
summary(Prediction_Errors)
summary(Prediction_Hold_Correct)
summary(Prediction_Break_Correct)
